...
class Module(Module):
    def process(self, interaction):
        ...
        res = self.rori.send_for_best_client("command", interaction.author_ring_id, "pactl set-sink-volume 0 100%", rmd)
        ...
        if res:
            string_to_say = self.rori.get_localized_sentence('unmute', self.sentences)
            self.rori.send_for_best_client("text/plain", interaction.author_ring_id, string_to_say, rmd)
        else:
